...
delivery_drone.metatable = {__index = delivery_drone}
...
delivery_drone.new = function(garage_depot, drone_name)

  local entity = garage_depot.entity.surface.create_entity{name = get_drone_name(drone_name), position = garage_depot.corpse.position, force = garage_depot.entity.force}

  local drone =
  {
    entity = entity,
    garage_depot = garage_depot,
    index = tostring(entity.unit_number),
    state = 0,
    requested_count = 0,
    tick_created = game.tick
  }
  setmetatable(drone, delivery_drone.metatable)
  add_drone(drone)
...
function delivery_drone:update_speed()
...
function delivery_drone:add_slow_sticker()
...
function delivery_drone:deliver_fuel(depot, amount)
...
function delivery_drone:retry_command()
  ...
  if self.state == states.going_to_demand then
    if self.demand_depot.entity.valid then
      self:go_to_depot(self.demand_depot, distance)
    else
      self:return_to_garage()
    end
    return
  ...
  if self.state == states.return_to_garage then
    if self.garage_depot.entity.valid then
      self:go_to_depot(self.garage_depot, distance)
    else
      self:suicide()
    end
    return
...
function delivery_drone:process_failed_command()
...
function delivery_drone:distance(position)
  return distance(self.entity.position, position)
end

local min = math.min
function delivery_drone:process_pickup()
  ...
  local available_count = self.requested_count + self.supply_depot:get_available_item_count(self.requested_item)

  local to_take
  if not self.demand_depot.circuit_limit then
    -- No circuit limit, pickup as much as we can
    to_take = min(available_count, self.requested_count)
  else
  ...
  self:add_slow_sticker()
  self:update_speed()
  self:return_to_demander(sprite_switch)

end

function delivery_drone:process_delivery()

  if not self.demand_depot.entity.valid then
    self:return_to_garage()
    return
  end

  local given_count = self.demand_depot:deliver_item(self.requested_item, self.held_count)
...
function delivery_drone:process_deliver_fuel()
  ...
  self:add_slow_sticker()
  self:update_speed()
  self:return_to_garage(delivered)

end

function delivery_drone:clear_reservations()

  if self.state == states.going_to_supply then
    if self.requested_item then
      if self.supply_depot and self.supply_depot.entity.valid then
        self.supply_depot:add_to_be_taken(self.requested_item, -self.requested_count)
      end
      if self.demand_depot and self.demand_depot.entity.valid then
        self.demand_depot:add_to_be_delivered(self.requested_item, -self.requested_count)
      end
...
function delivery_drone:return_to_demander(sprite_switch)
  ...
  self.state = states.going_to_demander

  self:go_to_depot(self.demand_depot, nil, sprite_switch)

end


function delivery_drone:return_to_garage(sprite_switch)

  self:clear_reservations()

  if not self.garage_depot.entity.valid then
    self:suicide()
    return
  end
  ...
  self.state = states.return_to_garage

  self:go_to_depot(self.garage_depot, nil, sprite_switch)
...
function delivery_drone:update_sticker()
...
function delivery_drone:suicide()
  self:say("S")
...
function delivery_drone:process_return_to_garage()
  ...
  -- if self.held_item then
  --   self.demand_depot:take_item(self.held_item, self.held_count, self.held_temperature)
  --   self.held_item = nil
  -- end

  if self.fuel_amount then
    self.garage_depot.entity.insert_fluid({name = get_fuel_fluid(), amount = self.fuel_amount})
    self.fuel_amount = nil
  end
...
local random = math.random
function delivery_drone:wait_for_reorder()
...
function delivery_drone:refund_fuel()
  ...
  self.garage_depot.entity.insert_fluid({name = get_fuel_fluid(), amount = fuel_refund})
  elseif fuel_refund < 0 then
    self.garage_depot.entity.remove_fluid({name = get_fuel_fluid(), amount = -fuel_refund})
  end

end

function delivery_drone:remove_from_depot()
...
local min = math.min
function delivery_drone:update(event)
  ...
  if self.state == states.going_to_demander then
  ...
  if self.state == states.return_to_garage then
    self:process_return_to_garage()
    return
  end

end

function delivery_drone:say(text)
  self.entity.surface.create_entity{name = "tutorial-flying-text", position = self.entity.position, text = text}
end

function delivery_drone:go_to_position(position, radius)
  self.entity.set_command
  {
...
function delivery_drone:go_to_entity(entity, radius)
  self.entity.set_command
  {
    type = defines.command.go_to_location,
...
function delivery_drone:go_to_depot(depot, radius, sprite_switch)
...
function delivery_drone:clear_drone_data()
...
function delivery_drone:handle_drone_deletion()
  ...
  if self.garage_depot.entity.valid then
    self.garage_depot:remove_drone(self, true)
  end
...
delivery_drone.on_load = function()
  script_data = global.delivery_drone or script_data
  for unit_number, drone in pairs (script_data.drones) do
    setmetatable(drone, delivery_drone.metatable)
  end
end

delivery_drone.on_init = function()
  global.delivery_drone = global.delivery_drone or script_data
  set_map_settings()
end

delivery_drone.on_configuration_changed = function()
  ...
  for k, drone in pairs (script_data.drones) do
    if drone.entity.valid then
      if drone.state == states.going_to_supply then
        ...
        local item = drone.requested_item
        drone:make_delivery(drone.supply_depot, drone.demand_depot, item, count)
      end
...
delivery_drone.get_drone_count = function()
